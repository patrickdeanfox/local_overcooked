...
- Pick the theme and fill the **New theme** column of `docs/THEME.md`, the noun map: every ingredient, cookware state, carrier, station, chef action, hazard, order term, level theme and kitchen sound, each with the sim rule its counterpart has to satisfy and the identifiers it renames. Rows marked **hard** (burning, dirty plates and the sink, the extinguisher, fire) have no natural counterpart outside a kitchen; a hard row the theme cannot answer is a mechanics change, not a reskin, so decide it before more levels build on that mechanic.
