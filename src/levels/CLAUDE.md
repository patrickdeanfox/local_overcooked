...
## Files
- `schema.ts` — the contract: `LevelDef` and its parts (`OrderSettings`, `LevelStars`, `PlateSettings`, `StationOverride`, `ItemPlacement`, the `Dynamic` union: `pedestrians`, `sliders`, `gate`), `LEGEND` (26 characters), `SOLID_TILES`, `isWalkable`, `makeItem`, `parseGrid`, `validateLevel`. Additive changes only.
...
- `oc1/1-1.json` to `1-6.json` — the six shipped levels. None uses the `stations` or `items` override arrays yet.
...
Worlds 2 to 6 and Overcooked 2 (`docs/ROADMAP.md` item 5), the `gap` tile (item 4), the generator `src/levels/generate.ts` and its thousand-seed test (item 8), the catalog-to-LevelDef converter (item 11).
