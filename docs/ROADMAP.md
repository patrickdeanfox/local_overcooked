...
1. **Needs nothing new:** OC1 3-2 shipped in PR #49 (a clean plate return with no sink as a `plateStack` override, two serving counters, a split deck; the reachability test now takes the spawns together). Plated dishes (`DishType 'plated'`, `PLATED_INGREDIENTS`), fish and prawn, the sashimi recipes and the generic `C` crate shipped in PR #50, which is `oc2-roadmap-notes.md` steps 1 and 2; OC2 1-1 follows as a level PR. The tutorial has no catalog record, so salad waits for cucumber.
