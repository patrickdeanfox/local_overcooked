...
- **The look.** Menus are a diner pass: enamel teal, cream tickets, mustard for the cursor, tomato for danger, in two self-hosted open-licence faces (Titan One for headings and numbers, Fredoka for everything else; `public/fonts/`). The title is a sidebar of level tickets over a living diorama: the selected kitchen is built on the 3D stage with your own chefs standing at their spawns, seen from the front-right and swaying slowly, with its name, theme, a line or two about the kitchen (paraphrased from the wiki, never the strategy) and your record on a card; the work button (Shift, or X on a pad) swaps the card to the wiki's strategy for that kitchen, and the pause menu's **Strategy** row shows it mid-game. Every page opens with a staggered slide-in; the results screen pops its stars in one by one, counts the score up and slams a "New best!" stamp. The pause menu lists the kitchen's buttons (pick up, work, throw, dash) in your own labels.
...
public/fonts/   the two OFL faces (Titan One, Fredoka) with their licences; index.html declares them and BootScene waits for them
