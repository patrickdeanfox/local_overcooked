...
- Theme chosen on 2026-09-09: **Potion Shop**, apprentices brewing draughts and elixirs in a guild workshop. `docs/THEME.md` is the filled noun map: every ingredient, cookware state, carrier, station, chef action, hazard, order term, level theme and kitchen sound, each with the sim rule its counterpart satisfies and the identifiers it renames. Every **hard** row has an answer (curdled batch and cauldron flare, crusted flask and rinse basin, dousing charm), so this is a straight reskin with no mechanics change. Rows marked **(proposed)** there want the user's word; the "Not in the clone yet" table names the stations and formulas item 5 has still to build, so each is named once when it lands.
...
## 14. Mechanics additions: two-plate carry, chop assist, the tray, the pass-through shelf, the 86 system — shipped 2026-09-09
The five additions specified in `docs/MECHANICS.md`, each behind a switch on the Settings page (`Settings.mechanics`, off by default so the shipped kitchens and their star thresholds are unchanged). Built contract first (additive members in `src/sim/types.ts`, `src/sim/constants.ts`, `src/levels/schema.ts`, `src/art/keys.ts`, `src/audio/types.ts`, `src/game/settings.ts`), then the arms in parallel: sim rules with a test file per mechanic, three new tiles (`shelf` `h`, `trayRack` `t`, `delivery` `d`) and the `tray` item drawn in 2D and 3D, eight sounds, the Settings rows, the HUD flash and chalk mark, and the levels. Levels: 1-4, 1-5 and OC2 1-1 gained crate limits and a delivery door through `stations` overrides and an `eightySix` block (the grid text stays the transcription), 1-5 a tray rack; `src/levels/custom/` holds Hatch Row (near-split soups through three hatches), Long Haul (a corridor for the tray) and Short Order (scripted shortages, two-plate carry). Section 9 of the spec is the code map.
