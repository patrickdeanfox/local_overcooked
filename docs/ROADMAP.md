...
## 4. Overcooked 2 dynamics: throwing and dashing — M, sim half done 2026-09-08
Shipped in PR #45 (sim arm): `TileType 'gap'` (legend `_`; no wall, a throw crosses it, a chef that steps on it falls and respawns after `FALL_PENALTY_SEC`), `PlayerInput.throwPressed? / dashPressed?`, `SimState.flying?`, `Chef.dashCooldown? / dashTimeLeft? / respawnIn?`, `ChefAction 'dashing' | 'falling'`, events `throw`, `catch`, `throwLand`, `dash`, `dashBump`, `chefFell`, and the rules below in `src/sim/index.ts` with `tests/sim.throw.test.ts`, `sim.dash.test.ts`, `sim.gap.test.ts`. Throws already fly over a closed 1-6 gate, so 1-6 has its drop with no level change. Input landed in PR #46: `GameAction 'throw' | 'dash'` (keyboard E / Q and / / ., pad Y and B), the two extra rows and wizard steps, `LATER_ACTIONS` so older saved maps keep their keys. Still to do, one arm each: audio (`SfxName` for the six events), presentation (flying items as an arc, a dash stretch and dust puff, the hole in the floor, a hidden falling chef), then the OC2 cheap list in `docs/research/oc2-roadmap-notes.md` section 15.
