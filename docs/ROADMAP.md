...
1. **Needs nothing new:** OC1 3-2 shipped in PR #49 (a clean plate return with no sink as a `plateStack` override, two serving counters, a split deck; the reachability test now takes the spawns together). OC2 Tutorial and 1-1 need only chop-and-plate dishes (salad, sashimi): see `oc2-roadmap-notes.md` step 1.
